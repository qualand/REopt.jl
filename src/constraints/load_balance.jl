...
function add_elec_load_balance_constraints(m, p; _n="") 
    ...
    if isempty(p.s.electric_tariff.export_bins)
        conrefs = @constraint(m, [ts in p.time_steps_with_grid],
            ...
            + sum(m[Symbol("dvHeatingProduction"*_n)][t, q, ts] / p.backup_heating_cop[t] for q in p.heating_loads, t in p.techs.electric_heater)
            + sum(m[Symbol("dvHeatingProduction"*_n)][t, q, ts] / p.heating_cop[t][ts] for q in p.heating_loads, t in p.techs.ashp) #need to add cooling
